...
# input_file = "hsu_mdai_training_data_question_test.jsonl"
input_file = "hsu_mdai_training_data_question_reduced_second_half.jsonl"  #"hsu_mdai_training_data_question_reduced.jsonl"
output_file = "hsu_mdai_training_data_question_pair.jsonl"
...
async def generate_response(session, data):
    ...
    payload = {
        "model": "gpt-4o-mini",  # 사용할 GPT 모델 지정 "gpt-4o-mini" "gpt-3.5-turbo"
        "messages": [
            {"role": "system", "content": PROMPT_FOR_LLM},
            {"role": "user", "content": data["prompt"]}  # GPT 모델이 응답 생성 시 사용할 프롬프트(모델에게 질문 또는 맥락 정보를 제공)
        ],
        # "prompt": data["prompt"],  # GPT 모델이 응답 생성 시 사용할 프롬프트(모델에게 질문 또는 맥락 정보를 제공)
    ...
    try:
        ...
        async with session.post("https://api.openai.com/v1/chat/completions", headers=headers,
            ...
            if response.status == 200:  # response.status는 openai 서버에서 받아온 응답 객체의 상태 코드
                ...
                data["completion"] = result["choices"][0]["message"]["content"].strip()
...
async def process_file(input_path, output_path, batch_size=20):  # batch_size=10
    ...
    async with aiohttp.ClientSession() as session:  # aiohttp.ClientSession은 HTTP 요청을 효율적으로 처리하는 객체(재사용 가능)
        for line in tqdm(lines, desc="Processing JSONL"):
            ...
            # 요청이 필요한 경우만 처리
            if not data.get("completion"):  # completion이 없는 데이터만 처리 대상(파이썬에서는 빈 문자열 ""도 False)
                tasks.append(generate_response(session, data))  # GPT API 요청하여 코루틴 객체로 만들고 task 리스트에 추가
...
def main():
    ...
    print("병렬 처리를 시작합니다...")
    asyncio.run(process_file(input_file, output_file, batch_size=20))
